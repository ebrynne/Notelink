...
class NotesCore(gtk.Window):
  ...
  def click_tag_text(self, textView, event, noteId):
    ...
    for tag in iter.get_tags():
      # Find the tag id for the tag that's been clicked
      ...
      if name.find("tagNF") == 0:
        ...
        text_buffer.delete(iter, iter2)

        # Save the tag changes to the DB
  ...
  def note_move_cursor(self, textview):
    ...
    # Update how text will be formatted, based on where the cursor has been placed
    ...
    if self.tagTab.lookup("delMark") in tags:
      viewLink = None
      for i in tags:
        if i.get_property("name").find("viewLink") == 0: viewLink = i
      if viewLink != None:
        iter2 = iter.copy()
        iter.backward_to_tag_toggle(viewLink)
        iter2.forward_to_tag_toggle(viewLink)
        ...
        inline_note_id = int(text[0])
        if text[1] == '1':
          noteOpen = iter.forward_search("<nl::noteInline::" + str(inline_note_id) + ">", gtk.TEXT_SEARCH_TEXT_ONLY)
          markOpen0 = textBuf.create_mark("mark0", noteOpen[0])
          markOpen1 = textBuf.create_mark("mark1", noteOpen[1])
          noteEnd = iter.forward_search("</nl::noteInlineContent::" + str(inline_note_id) + ">", gtk.TEXT_SEARCH_TEXT_ONLY)
          markEnd = textBuf.create_mark("markEnd", noteEnd[0])
          noteClose = iter.forward_search("</nl::noteInline::" + str(inline_note_id) + ">", gtk.TEXT_SEARCH_TEXT_ONLY)
          markClose = textBuf.create_mark("markClose", noteClose[1])
          inBuf = gtk.TextBuffer(self.tagTab)
          inBuf.insert_range(inBuf.get_start_iter(), textBuf.get_iter_at_mark(markOpen1), textBuf.get_iter_at_mark(markEnd))
          self.file_from_buf(inBuf, inline_note_id)
